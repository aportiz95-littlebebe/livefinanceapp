...
def render_unified_income_splits_modal():
    ...
    with col_inputs:
        ...
        new_starting_savings = st.number_input(
            ...
            step=100.0, 
            format="%.2f",
            help="Your existing savings before this year's tracking began."
        )
        ...
        if new_anchor == "Next Payday":
            ...
            st.caption(f"Calculated first payday of {target_year}: **{new_first_payday.strftime('%b %d, %Y')}**")
    ...
    with col_ledger:
        ...
        edited_inc_df = st.data_editor(
            ...
            num_rows="dynamic",
            column_config={
                "Effective Date": st.column_config.DateColumn(format="YYYY-MM-DD", required=True),
                "Amount": st.column_config.NumberColumn(format="$%.2f", min_value=0.0, required=True)
            },
    ...
    if val_needs + val_wants + val_savings == 100.0 or (val_needs == 0 and val_wants == 0 and val_savings == 0):
        ...
        st.error("Percentages must sum up to exactly 100% (or be 0) before committing.")
...
def render_combined_envelopes_modal():
    ...
    st.caption("Editing draft mode. No changes apply to your dashboard until you hit Save Changes.")
    
    def toggle_edit_buck(idx, state): 
        st.session_state[f"edit_buck_{idx}"] = state
    ...
    def save_buck(idx):
        ...
        if new_name != old_name and new_name.strip():
            ...
            if old_name in st.session_state.temp_bucket_targets:
                del st.session_state.temp_bucket_targets[old_name]
        else:
    ...
    def del_buck(idx):
        ...
        if name_to_del in st.session_state.temp_bucket_targets:
            del st.session_state.temp_bucket_targets[name_to_del]
        st.session_state.pop(f"edit_buck_{idx}", None)

    if st.session_state.temp_bucket_config:
        ...
        st.info("No active custom envelopes built yet. Use the option form fields below to instantiate one.")
    ...
    def add_bucket():
        ...
        if n and n not in st.session_state.temp_bucket_config:
            ...
            st.session_state["new_b_name"] = ""
            st.session_state["new_b_init"] = 0.0
            st.session_state["new_b_pct"] = 0.0
            st.session_state["new_b_tgt"] = 0.0
    ...
    with add_btn_col: 
        st.button("Add Savings Bucket", use_container_width=True, on_click=add_bucket, disabled=is_any_bucket_editing)
    ...
    if custom_unbacked_goals:
        st.markdown("---")
        st.markdown("### 🎯 Standalone Custom Milestones (No Automatic Inflows)")
        ...
        def save_custom_unbacked(key_name, idx):
            ...
            if new_n != key_name and new_n.strip():
                ...
                st.session_state.temp_bucket_targets[key_name] = new_v
    ...
    def add_standalone_goal():
        ...
        if n and n not in st.session_state.temp_bucket_targets:
            ...
            st.session_state["new_unbacked_name"] = ""
            st.session_state["new_unbacked_val"] = 0.0
    ...
    with add_unbk_col:
        st.button("Add Standalone Goal", use_container_width=True, on_click=add_standalone_goal, disabled=is_any_unbacked_editing)
